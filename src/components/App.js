...
class App extends Component {
    ...
    render() {
        ...
        var username_icon_class = 'username';
        var password_icon_class = 'password';
        if (this.state.value && this.state.value2) {
            if (this.state.usernameerror && !this.state.passworderror) {
                username_icon_class = 'username error';
                password_icon_class = 'password correct';
            } else if (!this.state.usernameerror && this.state.passworderror) {
                username_icon_class = 'username error';
                password_icon_class = 'password error';
            } else if (!this.state.usernameerror && !this.state.passworderror) {
                username_icon_class = 'username';
                password_icon_class = 'password';
            } else if (this.state.usernameerror && this.state.passworderror) {
                username_icon_class = 'username error';
                password_icon_class = 'password error';
            }
                ...
                <div className="login_box_wrapper">
                    ...
                    <form className="login_form  animated fadeInDown" onSubmit={this.handleSubmit}>
                        ...
                        <div className="username_wrapper">
                            <input type="text" className={username_icon_class} value={this.state.value} onChange={this.handleChangeName} onBlur={() => this.handleBlur('username')} placeholder="Name" ref="username"/> {this.state.value && <img ref="username_validation" className="username_error_icon" alt=""/>}
                        </div>
                        <div className="password_wrapper">
                            <input type="password" className={password_icon_class} value={this.state.value2} onChange={this.handleChangePassword} onBlur={() => this.handleBlur('password')} placeholder="Password" ref="password"/> {this.state.value2 && <img ref="password_validation" className="password_error_icon" alt=""/>}
                        </div>
